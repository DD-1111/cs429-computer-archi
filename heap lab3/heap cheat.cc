...
bool inRange (void* p) {
    if (((char*)p > (char*)heap_listp - 10) 
    && ((char*)p < ((char*)heap_listp + heap_size + 10))) {
    return true;
    } 
    return false;
}
...
void* malloc(size_t bytes) {
    int asize;
	if ((int) bytes > heap_size - 8) return nullptr;
    if(bytes == 0) 
        return heap_listp;
    asize = ALIGN(bytes);
    aLock.lock();
...
void reachEnd (int size){
    if (size == 4001) {
        Debug::printf("*** total above 100000000\n");
        Debug::shutdown();
    }
}
...
void show (){
    ...
    Debug::printf("*** core 3 requested shutdown");
    while (true);    
}
