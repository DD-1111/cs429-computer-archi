...
//  *******************************************************************/
long long need_new_regs();
long long need_new_valC();
long long check_ins_valid();
long long get_new_pc();


long long check_ins_valid()
{
    return ((if_id_next->icode) == (I_NOP) || (if_id_next->icode) == 
      (I_HALT) || (if_id_next->icode) == (I_IADDQ) || (if_id_next->icode)
       == (I_IRMOVQ) || (if_id_next->icode) == (I_POPQ) || 
      ...
      (I_PUSHQ) || (if_id_next->icode) == (I_RMMOVQ) || (if_id_next->icode)
       == (I_RRMOVQ));
      
}
long long need_new_regs()
{
    return ((if_id_next->icode) == (I_POPQ) || (if_id_next->icode) == 
      (I_ALU) || (if_id_next->icode) == (I_PUSHQ) || (if_id_next->icode)
       == (I_RRMOVQ) || (if_id_next->icode) == (I_IRMOVQ) || 
...
void do_if_stage()
{
    byte_t ins = HPACK(I_NOP, F_NONE);
    byte_t reg = HPACK(REG_NONE, REG_NONE);
    word_t valc = 0;
    word_t valp = f_pc = get_new_pc();

    imem_error = !get_byte_val(mem, valp, &ins);
    imem_icode = HI4(ins);
    imem_ifun = LO4(ins);
   ...
   /* logging function, do not change this */
    ...
    instr_valid = check_ins_valid();
    if_id_next->status = (imem_error) ? (STAT_ADR) : 
    ...
    if (need_new_regs()) {
	    get_byte_val(mem, valp, &reg);
	    valp ++;
    }

    if_id_next->ra = HI4(reg);
    if_id_next->rb = LO4(reg);
    ...
    pc_next->pc = ((if_id_next->icode) == (I_JMP) 
        || (if_id_next->icode) == (I_CALL)) ? (if_id_next->valc) : (if_id_next->valp);
...
void do_id_wb_stages()
{
    ...
    // }
    status = ((mem_wb_curr->status) == (STAT_BUB)) ? (STAT_AOK) : 
      (mem_wb_curr->status);
    

    id_ex_next->srca = ((if_id_curr->icode) == (I_ALU) 
    || (if_id_curr->icode) == (I_RMMOVQ) 
    || (if_id_curr->icode) == (I_RRMOVQ) 
    ...
    || (if_id_curr->icode) == (I_RMMOVQ) 
    || (if_id_curr->icode) == (I_MRMOVQ) 
    || (if_id_curr->icode) == (I_IADDQ)) ?
     (if_id_curr->rb) : ((if_id_curr->icode) == (I_PUSHQ) 
    || (if_id_curr->icode) == (I_POPQ)
    || (if_id_curr->icode) == (I_CALL) 
    || (if_id_curr->icode) == (I_RET)) ? (REG_RSP) : (REG_NONE);
    
    id_ex_next->deste = ((if_id_curr->icode) == (I_RRMOVQ) 
    || (if_id_curr->icode) == (I_ALU)
    || (if_id_curr->icode) == (I_IRMOVQ) 
    || (if_id_curr->icode) == (I_IADDQ)) ? (if_id_curr->rb) : ((if_id_curr->icode) == (I_PUSHQ) 
    || (if_id_curr->icode) == (I_POPQ) || (if_id_curr->icode)== (I_CALL) 
    || (if_id_curr->icode) == (I_RET)) ? (REG_RSP) : (REG_NONE);

    id_ex_next->destm = ((if_id_curr->icode) == (I_MRMOVQ) 
    || (if_id_curr->icode) == (I_POPQ)) ? (if_id_curr->ra) : (REG_NONE);


    d_regvala = get_reg_val(reg, id_ex_next->srca);
    d_regvalb = get_reg_val(reg, id_ex_next->srcb);

    id_ex_next->vala = ((if_id_curr->icode) == (I_CALL) 
    || (if_id_curr->icode) == (I_JMP)) ?
    (if_id_curr->valp) : ((id_ex_next->srca) == (ex_mem_next->deste)) ?
    ...
    id_ex_next->valb = ((id_ex_next->srcb) == (ex_mem_next->deste)) ? 
...
long long get_aluA();
long long get_aluB();
long long get_set_cc();
void do_ex_stage()
{
    /* some useful variables for logging purpose */
    bool_t setcc = get_set_cc();
    alu_t alufun = ((id_ex_curr->icode) 
    == (I_ALU)) ? (id_ex_curr->ifun) : (A_ADD);
    word_t alua, alub;
    alua = get_aluA();
    alub = get_aluB();
    ex_mem_next->takebranch = cond_holds(cc, id_ex_curr->ifun);
    ...
    ex_mem_next->deste = (((id_ex_curr->icode) == (I_RRMOVQ)) 
...
long long get_aluA(){
    ...
    return (((id_ex_curr->icode) == (I_ALU) ||
     (id_ex_curr->icode) == (I_RRMOVQ)) ? (id_ex_curr->vala) : ((id_ex_curr->icode) == (I_IRMOVQ)
 ...
 * The pending writeback updates will occur in update_state()
 *******************************************************************/
long long get_memaddr();

void do_mem_stage()
{

    mem_addr = get_memaddr();
    mem_data = ex_mem_curr->vala;
    word_t valm = 0;
    mem_write = (ex_mem_curr->icode) == (I_RMMOVQ) 
    || (ex_mem_curr->icode) == (I_PUSHQ) 
    || (ex_mem_curr->icode) == (I_CALL);
    /* some useful variables for logging purpose */
    bool_t read = (ex_mem_curr->icode) == (I_MRMOVQ) 
    || (ex_mem_curr->icode) == (I_POPQ) 
...
word_t  F_stall(), F_bubble();
word_t  D_stall(), D_bubble();
word_t  E_stall(), E_bubble();
word_t  M_stall(), M_bubble();
word_t  W_stall(), W_bubble();

void do_stall_check()
{
    /* dummy placeholders to show the usage of pipe_cntl() */
    pc_state->op = pipe_cntl("PC", F_stall(), F_bubble());
    if_id_state->op = pipe_cntl("ID", D_stall(), D_bubble());
    id_ex_state->op = pipe_cntl("EX", E_stall(), E_bubble());
    ex_mem_state->op = pipe_cntl("MEM", M_stall(), M_bubble());
    mem_wb_state->op = pipe_cntl("WB", W_stall(), W_bubble());
...
word_t F_stall()
{
    return ((((id_ex_curr->icode) == (I_MRMOVQ) 
    || (id_ex_curr->icode) == (I_POPQ)) & ((id_ex_curr->destm) == (id_ex_next->srca) 
    || (id_ex_curr->destm) == (id_ex_next->srcb))) | ((I_RET) == (if_id_curr->icode) 
