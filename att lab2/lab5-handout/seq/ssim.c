...
//
    ...
    imem_ifun = LO4(instr);
    icode = (imem_error) ? (I_NOP) : (imem_icode);
    ifun  = (imem_error) ? (F_NONE) : (imem_ifun);
    ins_valid = valid_check();
    valp++;
    if (more_regids()) {
	byte_t regids;
    ...
    destE = (((icode) == (I_RRMOVQ)) & (cond)) ? (rb) : ((icode) == 
        (I_IRMOVQ) || 
        (icode) == (I_ALU)) ? (rb) : ((icode) == (I_PUSHQ)||
        (icode) == (I_POPQ) ||
        (icode) == (I_CALL) || 
        (icode) == (I_RET)) ? 
        (REG_RSP) : (REG_NONE);
    destM = ((icode) == (I_MRMOVQ) || (icode) == (I_POPQ)) ? (ra) : 
      (REG_NONE);

    aluA = get_aluA();
    aluB = get_aluB();
    alufun = ((icode) == (I_ALU)) ? (ifun) : (A_ADD);
    ...
    mem_addr = ((icode) == (I_RMMOVQ) ||
                (icode) == (I_PUSHQ) ||
                (icode) == (I_CALL) || 
                (icode) == (I_MRMOVQ)) ? 
                (vale) : ((icode) == (I_POPQ)||
                (icode) == (I_RET)) ? (vala) : 0;
    mem_data = ((icode) == (I_RMMOVQ) ||
                (icode) == (I_PUSHQ)) ?
                (vala) : ((icode) == 
                (I_CALL)) ? (valp) : 0;
    /*********************** Memory stage ***********************
  ...
  if ((icode) == (I_MRMOVQ) ||
    ...
    status = ((imem_error) | (dmem_error)) ? (STAT_ADR) : !(ins_valid) ? 
    ...
    pc_in = ((icode) == (I_CALL)) ? (valc) : (((icode) == (I_JMP)) & (cond)
